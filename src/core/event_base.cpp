...
Key GetKey(int32_t GLFW_key)
{
	switch (GLFW_key)
	{
		...
		case GLFW_KEY_KP_ADD: return Key::NumAdd;
		case GLFW_KEY_KP_SUBTRACT: return Key::NumMinus;
		case GLFW_KEY_KP_MULTIPLY: return Key::NumMult;
		case GLFW_KEY_KP_DIVIDE: return Key::NumDivide;
		case GLFW_KEY_KP_DECIMAL: return Key::NumDecimal;
		case GLFW_KEY_KP_ENTER: return Key::NumEnter;
...
int32_t GetGLFWKey(Key key)
{
	...
	case Key::NumMinus: return GLFW_KEY_KP_SUBTRACT;
	case Key::NumMult: return GLFW_KEY_KP_MULTIPLY;
...
{
	ModifierFlags modifier_flags = Modifier::Null;
	if (mods & GLFW_MOD_SHIFT)
		modifier_flags = modifier_flags | Modifier::Shift;
	if (mods & GLFW_MOD_CONTROL)
		modifier_flags = modifier_flags | Modifier::Control;
	if (mods & GLFW_MOD_ALT)
		modifier_flags = modifier_flags | Modifier::Alt;
	if (mods & GLFW_MOD_SUPER)
		modifier_flags = modifier_flags | Modifier::Super;
	if (mods & GLFW_MOD_CAPS_LOCK)
		modifier_flags = modifier_flags | Modifier::CapsLock;
	if (mods & GLFW_MOD_NUM_LOCK)
		modifier_flags = modifier_flags | Modifier::NumLock;
	return modifier_flags;
}

uint32_t GetGLFWModifiers(ModifierFlags mods)
{
	uint32_t glfw_mods = 0;
	if ((mods & Modifier::Shift) != Modifier::Null)
		glfw_mods |= GLFW_MOD_SHIFT;
	if ((mods & Modifier::Control) != Modifier::Null)
		glfw_mods |= GLFW_MOD_CONTROL;
	if ((mods & Modifier::Alt) != Modifier::Null)
		glfw_mods |= GLFW_MOD_ALT;
	if ((mods & Modifier::Super) != Modifier::Null)
		glfw_mods |= GLFW_MOD_SUPER;
	if ((mods & Modifier::CapsLock) != Modifier::Null)
		glfw_mods |= GLFW_MOD_CAPS_LOCK;
	if ((mods & Modifier::NumLock) != Modifier::Null)
		glfw_mods |= GLFW_MOD_NUM_LOCK;
	return glfw_mods;
}

KeyEvent::KeyEvent(EventHandler* event_handler, int32_t GLFW_key, int32_t GLFW_action, uint32_t GLFW_mods): IEvent(event_handler)
{
	key = GetKey(GLFW_key);
	action = GetAction(GLFW_action);
	mods   = GetModifiers(GLFW_mods);
}

MouseButtonEvent::MouseButtonEvent(EventHandler* event_handler, int32_t GLFW_button, int32_t GLFW_action, uint32_t GLFW_mods):
	IEvent(event_handler)
{
	button = GetMouseButton(GLFW_button);
	action = GetAction(GLFW_action);
	mods   = GetModifiers(GLFW_mods);
}

MouseMoveEvent::MouseMoveEvent(EventHandler* event_handler, double GLFW_x, double GLFW_y):
	IEvent(event_handler), pos(GLFW_x, GLFW_y)
{
}
